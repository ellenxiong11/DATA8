...
class Insect(object):
    """An Insect, the base class of Ant and Bee, has   and a Place."""
...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        # BEGIN EC
        move = self.place.exit
        if self.is_scared and not self.already_scared:
            move=self.place.entrance
        # END EC
        if self.blocked():
            self.sting(self.place.ant)
        elif self.armor > 0 and move is not None:
            self.move_to(move)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        throwing = self.place
        for i in range(self.min_range): #start position
            throwing = throwing.entrance
        i = 0
        while throwing.entrance and i <= self.max_range:
            i += 1
            if throwing.bees:
                return random_or_none(throwing.bees)
            throwing = throwing.entrance
        return None
        # END Problem 3 and 4
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        ...
        bees = self.place.bees
        if bees != []:
            [i.reduce_armor(self.damage) for i in bees[:]]
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        return (not self.contained_ant and not other.is_container) or False
        # END Problem 9
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        if self.contained_ant != None:
            self.contained_ant.action(colony)
        if self.place.bees != []:
            [i.reduce_armor(self.damage) for i in self.place.bees[:]]
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if self.first == False:
            ...
            temp_exit = self.place.exit
            while temp_exit:
                if temp_exit.ant is not None:
                    if temp_exit.ant.is_container:
                        contained = temp_exit.ant.contained_ant 
                        if contained not in self.doubled and contained:
                            contained.damage *= 2
                            self.doubled.append(contained)
                    if temp_exit.ant not in self.doubled:
                        temp_exit.ant.damage *= 2 
                        self.doubled.append(temp_exit.ant)
                temp_exit = temp_exit.exit
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    #action1 = bee.action
    #action2 = effect(bee.action, bee)
    """if duration>0:
                    bee.action=effect(bee.action, bee)
                duration -= 1"""

    original_action = bee.action
    with_affect = effect(bee.action, bee)
