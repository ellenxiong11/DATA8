...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    # All non-atomic expressions are lists (combinations)
        ...
        if scheme_symbolp(first) and first in SPECIAL_FORMS:
            ...
            return scheme_apply(procedure, rest.map(lambda input: scheme_eval(input, env)) , env)
            #END PROBLEM 4
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if (formals is nil) and (vals is nil):
            return child_frame
        raise SchemeError('wrong number of values/parameters')
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(target_first, do_lambda_form(Pair(target.rest, expressions.rest), env))
        return target_first
        # END PROBLEM 9
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    computed = scheme_eval(expressions.first, env, expressions.rest == nil)
    return computed if expressions.rest == nil or scheme_falsep(computed) else do_and_form(expressions.rest, env)'''

    if expressions is nil:
            return True
    """Evaluate a (short-circuited) and form."""
    def and_helper(expressions, env, rv):
        rv = scheme_eval(expressions.first, env, expressions.rest == nil)
        if not scheme_truep(rv):
            return False
        elif expressions.rest is nil:
            return rv
        return and_helper(expressions.rest, env, rv)


    return and_helper(expressions, env, True)
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    def or_helper(expressions, env, rv):
        rv = scheme_eval(expressions.first, env, expressions.rest == nil)
        if scheme_truep(rv):
            return rv
        elif expressions.rest is nil:
            return False
        return or_helper(expressions.rest, env, rv)
    return or_helper(expressions, env, False)
    # BEGIN PROBLEM 12
...
def make_let_frame(bindings, env):
    ...
    formal, param = nil, nil
    while bindings is not nil:
        seq_bindings = bindings.first
        check_form(seq_bindings, 2, 2)
        formal = Pair(seq_bindings.first, formal)
        param =  Pair(scheme_eval(seq_bindings.rest.first, env), param)
        bindings = bindings.rest

    # BEGIN PROBLEM 14
    check_formals(formal)
    let_frame = env.make_child_frame(formal, param)
    return let_frame
    # END PROBLEM 14
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 20
    check_form(expressions, 2)
    procedure = expressions.first
    if not isinstance(procedure, Pair):
        raise SchemeError("invalid define_macro format")
    else:
        name, params, body = procedure.first, procedure.rest, expressions.rest
        if scheme_symbolp(name) and not check_formals(params):
            env.define(name, MacroProcedure(params, body, env))
            return name
        raise SchemeError("invalid define_macro format")
